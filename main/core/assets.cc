...
bool Assets::Download(std::string url, std::function<void(int progress, size_t speed)> progress_callback) {
    ...
    // Unmap the current assets partition memory mapping
    UnApplyPartition();
    ...
    // Calculate the number of sectors that need to be erased
    ...
    // Write the new assets file to the partition, erasing as we go
    ...
    while (true) {
        ...
        // Erase the new sectors that are needed
        ...
        if (esp_timer_get_time() - last_calc_time >= 1000000 || total_written == content_length || ret == 0) {
            ...
            recent_written = 0; // Reset recently written byte count
        }
    }
