...
int NoAudioCodec::Write(const int16_t* data, int samples) {
    ...
    for (int i = 0; i < samples; i++) {
        int64_t temp = int64_t(data[i]) * volume_factor; // Use int64_t to avoid overflow during multiplication
        if (temp > INT32_MAX) {
            buffer[i] = INT32_MAX;
...
int NoAudioCodecSimplexPdm::Read(int16_t* dest, int samples) {
    ...
    // PDM-demodulated data is 16 bits wide, read directly into the destination buffer
