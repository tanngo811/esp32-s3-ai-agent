...
private:
    ...
    struct context_t {
        ...
        size_t bytes_needed = 0;        // Bytes still needed to finish reading the current field
        size_t seg_remaining = 0;       // Bytes remaining to read for the current segment
        size_t body_size = 0;           // Total size of the data body
        size_t body_offset = 0;         // Bytes already read of the data body
    };
...
public:
    ...
    /// @brief Set the callback invoked when demuxing is finished
