        ...
        switch (state_) {
          case ParseState::FIND_PAGE: {
            ...
            if (ctx_.bytes_needed < 4) {
                // Handle incomplete "OggS" match (spanning chunks)
                ...
                // Search for a full "OggS" within the data chunk
                bool found = false;
                size_t i = 0;
                ...
                if (found) {
                    ...
                    // Don't record the found "OggS"; not necessary
          ...
          case ParseState::PARSE_HEADER: {
            ...
            if (available < ctx_.bytes_needed) {
                ...
                if (ctx_.seg_count > 0 && ctx_.seg_count <= 255) {
                    ...
                    // No segments, jump to the next page
                    state_ = ParseState::FIND_PAGE;
                    ctx_.bytes_needed = 4;
                    ctx_.data_offset = 0;
                } else {
          ...
          case ParseState::PARSE_SEGMENTS: {
            ...
            if (available < ctx_.bytes_needed) {
                ...
                // Compute the total size of the data body
                ctx_.body_size = 0;
          ...
          case ParseState::PARSE_DATA: {
                ...
                if (!seg_continued) {
                    ...
                    if (ctx_.packet_len) {
                        ...
                        if (opus_info_.head_seen && opus_info_.tags_seen) {
                            ...
                            ESP_LOGW(TAG, "Current Ogg container has no OpusHead/OpusTags parsed, dropping");
                        }
                    }
                    ctx_.packet_len = 0;
                    ctx_.packet_continued = false;
            ...
            if (ctx_.seg_index == ctx_.seg_count) {
                // Check whether the entire data body has been read
                if (ctx_.body_offset < ctx_.body_size) {
                    ESP_LOGW(TAG, "Data body incomplete: %zu/%zu",
