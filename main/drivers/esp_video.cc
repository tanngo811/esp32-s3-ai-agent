...
EspVideo::EspVideo(const esp_video_init_config_t& config) {
    ...
    // Note: in the current esp_video version, YUV422P actually outputs as YUYV.
    ...
    // When ISP is enabled it needs a few frames to initialize its parameters, so capture and discard frames for 5s in the background after start
    xTaskCreate(
        [](void* arg) {
...
bool EspVideo::Capture() {
        ...
        if (i == 2) {
            ...
            switch (sensor_format_) {
                ...
                case V4L2_PIX_FMT_RGB565X: {
                    ...
                    // Currently esp_video always reports RGB565 (in either endianness) and never RGB565X; this case is here for future-version compatibility
                    auto src16 = (uint16_t*)mmap_buffers_[buf.index].start;
                    auto dst16 = (uint16_t*)frame_.data;
    ...
    if (display != nullptr) {
        ...
        switch (frame_.format) {
            // LVGL appears to have issues displaying YUV-family images; convert to RGB565 for display for now
            case V4L2_PIX_FMT_YUYV:
            case V4L2_PIX_FMT_YUV420:
            case V4L2_PIX_FMT_RGB24: {
            ...
            case V4L2_PIX_FMT_RGB565:
                ...
                lvgl_image_size = frame_.len;  // covers both YUYV and RGB565 on fallthrough
                break;
 ...
 *
 * This function encodes the image currently in the camera buffer as JPEG and sends it
 * via HTTP POST as multipart/form-data to the configured explain server. The server
 * runs AI analysis on the image based on the supplied question and returns the result.
 *
 * Implementation notes:
 * - JPEG encoding runs on a dedicated thread, separate from the main thread
 * - Uses chunked transfer encoding to keep memory usage low
 * - A queue synchronizes data between the encoder thread and the sender thread
 * - Supports HTTP headers for device ID, client ID, and authentication token
 *
 ...
 *         Example formats: {"success": true, "result": "<analysis result>"}
 *                          {"success": false, "message": "<error message>"}
 *
 * @note SetExplainUrl() must be called before calling this function to set the server URL
 * @note The function waits for any previous encoder thread to finish before starting a new request
