...
AdcBatteryMonitor::AdcBatteryMonitor(adc_unit_t adc_unit, adc_channel_t adc_channel, float upper_resistor, float lower_resistor, gpio_num_t charging_pin)
    ...
    // Conditional configuration for the ADC
    if (charging_pin_ != GPIO_NUM_NC) {
        ...
        // Do not set a callback, so the adc_battery_estimation library uses software estimation
...
bool AdcBatteryMonitor::IsCharging() {
    // Prefer using the adc_battery_estimation library's functionality
    ...
    // Fall back to GPIO read or return a default value
    if (charging_pin_ != GPIO_NUM_NC) {
...
uint8_t AdcBatteryMonitor::GetBatteryLevel() {
    ...
    if (err != ESP_OK) {
        return 100; // Return the default value on error
    }
