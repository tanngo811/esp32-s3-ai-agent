...
int Sy6970::GetBatteryLevel() {
    int level = 0;
    // Minimum voltage the battery is allowed to discharge to
    ...
    // When no battery is connected, the charging status is unstable and battery_voltage can sometimes exceed charge_voltage_limit
    if (level > 100) {
        level = 100;
    }
    return level;
